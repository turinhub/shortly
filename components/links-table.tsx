...
export function LinksTable() {
  ...
  return (
    <>
              ...
              <table className="w-full">
                <thead className="bg-muted/50">
                  <tr className="border-b border-border">
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">短链接</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">标题</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">原始链接</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">标签</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">点击量</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">状态</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">创建日期</th>
                ...
                <tbody>
                  {mockLinks.map((link) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <Link href={`/links/${link.id}`} className="font-medium hover:text-primary transition-colors">
                          {link.title}
                        </Link>
                        {link.description && (
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                              <Link href={`/links/${link.id}`} className="flex items-center gap-2 cursor-pointer">
                                <BarChart3 className="h-4 w-4" />
                                查看详情
                              </Link>
                            </DropdownMenuItem>
                            <DropdownMenuItem className="flex items-center gap-2" onClick={() => handleEdit(link)}>
      ...
      {editingLink && <EditLinkDialog link={editingLink} open={editDialogOpen} onOpenChange={setEditDialogOpen} />}
    </>
  )
}
